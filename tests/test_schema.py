...
from sqlalchemy.orm import relationship, synonym, column_property, aliased
...
class TestsSQLAlchemySchemaNode(unittest.TestCase):
    ...
    def test_declarative_overrides_class(self):
        Model = self._create_declarative_class()
        key = SQLAlchemySchemaNode.ca_class_key
        setattr(Model, key, {"overrides": {"name": {"typ": colander.Integer}}})
        model_schema = SQLAlchemySchemaNode(Model)
    ...
    def test_imperative_relationships_overrides(self):
        ...
        overrides = {"addresses": {"overrides": {"id": {"typ": colander.Float}}}}
        overrides = {"addresses": {"overrides": {"id": {"typ": colander.String}}}}
        schema = SQLAlchemySchemaNode(Person, overrides=overrides)
        self.assertIsInstance(schema["addresses"].children[0]["id"].typ, colander.String)
    ...
    def test_declarative_relationships_overrides(self):
        ...
        # Fake model to avoid a race condition
        dummy2 = WrongOverrides()  # noqa: F841
        ...
        class IncludesOverrides(Base):
            ...
            name = Column(Unicode(32), primary_key=True)
            model_id = Column(Unicode(32), ForeignKey("models.name"))
            model = relationship(Model, info={key: {"includes": ["name"]}})

        # Fake model to avoid a race condition
        dummy3 = IncludesOverrides()  # noqa: F841
        ...
        class ExcludesOverrides(Base):
            ...
            name = Column(Unicode(32), primary_key=True)
            model_id = Column(Unicode(32), ForeignKey("models.name"))
            model = relationship(Model, info={key: {"excludes": ["name"]}})
        ...
        self.assertIsInstance(schema["addresses"].children[0]["id"].typ, colander.Float)

    def _prep_schema(self):
        overrides = {
            "person": {
                "includes": ["name", "surname", "gender", "addresses", "phone_numbers"],
                "overrides": {
                    "addresses": {
                        "includes": ["street", "city"],
                        "overrides": {"city": {"exclude": False}},
                    }
                },
            },
        }
    ...
    def test_objectify(self):
        ...
        dict_ = {
            "person": {
                "gender": "M",
                "surname": "My Surname",
                "addresses": [{"city": "My City", "street": "My Street"}],
                "name": "My Name",
    ...
    def test_objectify_context(self):
        ...
        dict_ = {"enabled": True, "email": "mailbox@domain.tld"}
        schema = self._prep_schema()
    ...
    def test_clone(self):
        ...
        self.assertEqual(
            [node.name for node in schema.children],
    ...
    def test_missing_mapping_configuration(self):
        ...
        deserialized = schema.deserialize({"email": "test@example.com", "timeout": "09:44:33"})
    ...
    def test_relationship_declarative_configuration(self):
        ...
        self.assertEqual(address_mapping.description, "A location associated with a person.")
    ...
    def test_doc_example_deform(self):
        ...
        def generate_colander():
            class Phone(colander.MappingSchema):
                ...
                number = colander.SchemaNode(colander.String(), validator=colander.Length(0, 128))
            ...
            class Person(colander.MappingSchema):
                ...
                name = colander.SchemaNode(colander.String(), validator=colander.Length(0, 128))
                surname = colander.SchemaNode(colander.String(), validator=colander.Length(0, 128))
    ...
    def test_doc_example_less_boilerplate(self):
        ...
        def generate_colander():
            ...
            class Person(colander.MappingSchema):
                ...
                age = colander.SchemaNode(colander.Int(), missing=colander.null)
                phones = Phones(missing=[])
    ...
    def test_specify_order_fields(self):
        ...
        schema = SQLAlchemySchemaNode(MyClass, includes=["foo", "job_status", "id"])
        self.assertEqual(["job_status", "id"], [x.name for x in schema])
        self.assertNotIn("foo", schema)

        schema = SQLAlchemySchemaNode(MyClass, includes=["id", "foo", "job_status"])
    ...
    def test_non_text_includes(self):
        ...
        typ = colander.String()
        column = colander.SchemaNode(typ, name="customfield")
        schema = SQLAlchemySchemaNode(MyClass, includes=["foo", "job_status", column, "id"])
        self.assertEqual(["job_status", "customfield", "id"], [x.name for x in schema])
        self.assertNotIn("foo", schema)
    ...
    def test_typedecorator_overwrite(self):
        ...
        class World(Base):
            ...
            number = Column(MyIntType, info={key: {"typ": colander.Integer}})
            name = Column(MyString(5))

        world_schema = SQLAlchemySchemaNode(World)

        self.assertIsInstance(world_schema["not_a_number"].typ, colander.String)
    ...
    def test_example_typedecorator_overwrite(self):
        ...
        def generate_colander():
            class User(colander.MappingSchema):
                ...
                second_email = colander.SchemaNode(colander.String(), validator=colander.Email(), missing=colander.null)
    ...
    def test_default_typedecorator_override(self):
        ...
        self.assertRaises(ValueError, SQLAlchemySchemaNode, Numbers, None, None, None)
    ...
    def test_typedecorator_override_name(self):
        ...
        self.assertRaises(ValueError, SQLAlchemySchemaNode, BadTable, None, None, None)
    ...
    def test_relationship_circular_dependencies(self):
        ...
        overrides = {"cycle": {"overrides": {"cycle": {"includes": ["cycle"]}}}}
    ...
    def test_relationship_infinite_recursion(self):
        ...
        schema = SQLAlchemySchemaNode(Bar)  # noqa: F841
        schema = SQLAlchemySchemaNode(Baz)  # noqa: F841
